package com.lihao.service.serviceImpl.studentService;
...
public class StudentServiceImpl implements StudentService {
    ...
    @Override
    public List<User> select_all_student(Page page,String query) {
        PageHelper.startPage(page.getPage_num(), page.getPage_size());
        return userDao.select_all_student(query);
    }

    @Override
    public String insert_student(User u, MultipartFile file) throws Wrong {
        if(!u.getGender().equals("男") &&
                !u.getGender().equals("女")
        ){
            throw new Wrong(StringUtil.getInsert_fail());
        }
        String ss[] = FileUtil.fileBookLoad(file,StringUtil.getStudent_root());
        if(ss!=null){
            UserCategory userCategory = new UserCategory(1L,"学生");
            if(u.getPassword().isEmpty()){
                throw new Wrong("密码为空！");
            }
    ...
    @Override
    public String delete_student(UserIdDto userIdDto) throws Wrong {
    ...
    @Override
    public String update_student_blacklist(UserBlacklistDto userBlacklistDto) throws Wrong {
    ...
    @Override
    public String update_student(User user,MultipartFile file) throws Wrong {
        ...
        if(user.getUser_account_num()!=null&&
                ...
                user.getUser_account_num().charAt(0)!='s'){
            throw new Wrong(StringUtil.getUpdate_fail());
        }
        String[] ss = null;
        if(file!=null){
            ss=FileUtil.fileBookLoad(file,StringUtil.getStudent_root());
            if(ss==null){
                throw new Wrong(StringUtil.getUpdate_fail());
            }
        }
    ...
    @Override
    public User select_student(Long user_id) {
