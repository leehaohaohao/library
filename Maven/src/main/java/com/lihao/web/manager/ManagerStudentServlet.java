...
import com.lihao.service.StudentService;
...
public class ManagerStudentServlet {
    @Autowired
    private StudentService studentServiceImpl;
    @Autowired
    private UserDao userDao;
    @RequestMapping("/list")
    public ResponsePack<PageInfo<User>> Student_list(@ModelAttribute("page_num")Page page, @RequestParam("query")String query){
        ResponsePack<PageInfo<User>> responsePack = new ResponsePack<>();
        responsePack.setData(new PageInfo<>(studentServiceImpl.select_all_student(page,query)));
        responsePack.setSuccess(true);
        return responsePack;
    }
    @RequestMapping("/add")
    public ResponsePack<String> Student_add(String gender , String name , String user_num,
        ...
        responsePack.setData(studentServiceImpl.insert_student(user,file));
        responsePack.setSuccess(true);
        return responsePack;
    }
    ...
    public ResponsePack<String> Student_delete(@RequestBody UserIdDto userIdDto) throws Wrong {
        ResponsePack<String> responsePack = new ResponsePack<>();
        responsePack.setData(studentServiceImpl.delete_student(userIdDto));
        responsePack.setSuccess(true);
        return responsePack;
    }
    @PostMapping("/blacklist")
    public void Student_blacklist(@RequestBody UserBlacklistDto userBlacklistDto) throws Wrong {
        studentServiceImpl.update_student_blacklist(userBlacklistDto);
    }
    @GetMapping("/select")
    public ResponsePack<User> Student_select(@RequestParam Long user_id){
        ResponsePack<User> responsePack = new ResponsePack<>();
        responsePack.setData(studentServiceImpl.select_student(user_id));
        responsePack.setSuccess(false);
        return responsePack;
    }
    public User buildUser(Long user_id, String gender, String name, String telephone_num, String email, String user_account_num, BigDecimal fee_all, Boolean blacklist, String password) throws Wrong {
        if (user_id == null ) {
    ...
    public ResponsePack<String> Student_update(Long user_id, String gender, String name,
        ...
        ResponsePack<String> responsePack = new ResponsePack<>();
        responsePack.setData(studentServiceImpl.update_student(user,file));
        responsePack.setSuccess(true);
        return responsePack;
    }
    ...
    public void export(HttpServletResponse response) throws InstantiationException, IllegalAccessException, Wrong, IOException {
        ...
        List<User> userList = userDao.select_all_student("");
        Workbook workbook = excelUtil.deal("Students",userList,names);
